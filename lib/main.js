...
function create_server(connection_handler){
    ...
    function listen(port){
        console.log(`[${((new Date()).toLocaleDateString())} ${(new Date()).toLocaleTimeString()}] INFO  POP3 Server listening on 0.0.0.0:${port}`);
        socket.listen(port);
    }
...
function create_pop_connection(socket_connection){
    ...
    socket_connection.on('data', function(data){
        ...
        if(cmd.name === 'CAPA'){
            ...
            pop_connection.emit('dele', filter_index, function(deleted){
        ...
        else if(cmd.name === 'STLS'){
            pop_connection.emit('stls');
        }
        else{
            // TODO fail
...
function bind_pop3_commands(pop_connection){
    ...
    pop_connection.on('capa', function(){
        this.send_line('+OK Capability list follows');
        this.send_line('USER');
        this.send_line('UIDL');
        this.send_line('TOP');
        this.send_line('DELE');
    ...
    pop_connection.on('stls', function(){
        this.send_line('-ERR STLS not supported by server');
    });
